...
public class MVCTools : EditorWindow
{
    ...
    public static string newDesFileStr = "模块描述";
    ...
    public void OnGUI()
    {
        ...
        GUILayout.Space(10);
        GUILayout.Label("添加模块的名称描述");
        newDesFileStr = GUILayout.TextArea(newDesFileStr, 100);
        GUILayout.Space(10);
    ...
    private static void CreatWriteConfig()
    {
        if (File.Exists(GetConfigPath))
        {
            ...
            string CodeContent = "--" + newDesFileStr + "\r\n\t" + newUIStr + "=require " + "\"UI." + newUIStr + "." + newUIStr + "Config\"," + "\r\n\t"
                   + "--AppendCode";
            if (str.Contains(appendText))
            {
                str = str.Replace(appendText, CodeContent);
            }
            WriteToFile(GetConfigPath, str);
        }
    }
    ...
    private static void CreaWriteWindowsName()
    {
        if (File.Exists(GetUIWindowsPath))
        {
            ...
            WriteToFile(GetUIWindowsPath, str);
        }
    }
    ...
    private static void CreatUIScript(string BaseFilePath, string str)
    {
        ...
        {
            WriteToFile(BaseFilePath, str);
        }
    }

    public static void WriteToFile(string BaseFilePath, string str)
    {
    ...
    [MenuItem("Assets/删除MVC模块文件夹")]
    public static void DeleteMVCFolder()
    {
        UnityEngine.Object tmpObj = Selection.activeObject;
        string path = AssetDatabase.GetAssetPath(tmpObj);
        if (Directory.Exists(path))//判断选中的是否是文件夹
        {
            if (Directory.GetParent(path).Name.CompareTo("UI") == 0)
            {
                string tmpName = tmpObj.name;
                if (File.Exists(GetConfigPath))
                {
                    string str = ReadUITemp(GetConfigPath);
                    string CodeContent = tmpName + "=require " + "\"UI." + tmpName + "." + tmpName + "Config\",";
                    if (str.Contains(CodeContent))
                    {
                        str = str.Replace(CodeContent, "");
                    }
                    WriteToFile(GetConfigPath, str);

                    string strUIWindow = ReadUITemp(GetUIWindowsPath);
                    string uiWindow = tmpName + " = " + "\"" + tmpName + "\",";
                    if (strUIWindow.Contains(uiWindow))
                    {
                        strUIWindow = strUIWindow.Replace(uiWindow, "");
                    }
                    WriteToFile(GetUIWindowsPath, strUIWindow);
                }
            }
    ...
    [MenuItem("Assets/解析Map数据CSV")]
    public static void ReadMapCSV()
    {
        UnityEngine.Object tmpObj = Selection.activeObject;
        string path = AssetDatabase.GetAssetPath(tmpObj);
        string[] pathinfo = path.Split('/');
        string strpath = pathinfo[pathinfo.Length - 1];
        string[] strpathinfo = strpath.Split('.');
        string pathstr = strpathinfo[0];
        UnityEngine.Debug.Log(pathstr);
        if (strpath.EndsWith(".csv"))
        {
            string itemdata = "";
            itemdata = "local " + pathstr + " = {";
            string info = Resources.Load<TextAsset>(pathstr).text;
            string[] itemlist = info.Split('\n');//按行分割

            string[] name = itemlist[0].Split(',');//逗号分割
            string[] des = itemlist[1].Split(',');//逗号分割
            ...
            for (int i = 2; i < itemlist.Length; i++)
            {
                ...
                string[] b = itemlist[index].Split(',', '\r');
                index++;
            ...
            itemdata += "}\r\n return " + pathstr;
            Debug.Log(itemdata);
            File.WriteAllText("Assets/LuaScripts/Config/Data/" + pathstr + ".lua", @itemdata);
            Debug.Log("=====");
            AssetDatabase.Refresh();
        }

    }
    [MenuItem("Assets/解析CSV")]
        ...
        if (strpath.EndsWith(".csv"))
        {
            ...
            for (int i = 2; i < itemlist.Length; i++)
            {
                ...
                for (int j = 0; j < name.Length; j++)
                {
                    ...
                    if (j != name.Length - 1)
                    {
                        a = a + name[j] + "=" + "'" + b[j] + "'" + "," + "--" + des[j] + "\n";
                    }
                    else
                    {
                        string c = name[j] + "=" + "'" + b[j] + "'" + "," + "--" + des[j] + "\n";
                        c.Replace("\r", "");
                        a += c;
                    }
                }

...
//		_DOFStrength("DOF Strength", Float) = .5
		...
		//_adapted_lum("Fog Strength", Float) = .5
	}
	...
	SubShader 
	{
		...
		Pass {	//[Pass 0] Bloom/DOF final composite
			...
			#pragma multi_compile MOBILEDOF_ENABLED MOBILEDOF_DISABLED
			...
			#ifdef MOBILEDOF_ENABLED
			sampler2D _MobileDOFTex;
			float _DOF_FadeInFadeOut;
			//sampler2D _COCTex;
			#endif
			...
			fixed4 frag ( v2f_img_aa i ) : COLOR
			{
				...
				#ifdef MOBILEDOF_ENABLED
				fixed4 MobileDofTex = tex2D(_MobileDOFTex, i.uv2);
				fixed3 srcTex = MobileDofTex.rgb;
				srcTex = lerp(mainTex, srcTex, MobileDofTex.a*_DOF_FadeInFadeOut);
				#else
					fixed3 srcTex = mainTex;
				#endif
				...
				fixed filmGrain = 1;//dot(filmGrainTex, _FilmGrainChannel);
